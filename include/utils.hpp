...
template <typename T, typename E> inline T ok_or(std::optional<T> opt, E&& err) {
    if (!opt) {
        std::forward<E>(err)();
    }
    return *opt;
...
template <typename T> inline T ok_or_throw_str(std::optional<T> opt, const std::string& err) {
    if (!opt) {
...
template <typename T> inline T ptr_ok_or_throw_str(T pointer, const std::string& err) {
    if (!pointer) {
        throw std::runtime_error(err);
    }
    return pointer;
...
template <typename P, typename E> inline E ptr_unwrap_or(P pointer, const E other) {
    if (!pointer) {
        return other;
    }
}
